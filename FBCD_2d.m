...
%  This script is used for Fourier Domain
%
% INPUT: 
...
function [Mu, alpha, B1, B2, history] = FBCD_2d( F_1d, G , Y, D, c, d )
    ...
    fun_FTAF = @(X,alpha) reshape((alpha*( F_1d'*( G.^2.*( F_1d*X*(F_1d') ) )*F_1d )), [], 1); % function that gives the value of F^TAFx 
    fun_RTBR = @(X,B1,B2) reshape((( D'*( B1.*(D*X) ) ) + ( ( B2.*(X*(D')) )*D )), [], 1); % function that gives the value of R^TBRx
    fun_G = @(X,alpha,B1,B2) fun_FTAF(X,alpha) + fun_RTBR(X,B1,B2); % function that gives value of Gx
    fun_b = @(Y,alpha) reshape((alpha*( F_1d'*(G'.*Y)*F_1d )), [], 1); % function that gives the value of b = F^TAy 
    ...
    for counter = 1:MAX_ITER
        ...
        alpha = ( m^2 + 2*c )./( norm( reshape((( G.*(F_1d*Mu*(F_1d'))- Y )), [], 1) )^2 + 2*d );   
        ...
        history.abs_error(counter) = norm( reshape((( Mu-Mu_OLD )), [], 1) )^2;%/length(vec(Mu)); % absolute error 
        history.rel_error(counter) = ( norm( reshape((( Mu-Mu_OLD )), [], 1) )/norm( reshape((( Mu_OLD )), [], 1) ) )^2; % relative error        
