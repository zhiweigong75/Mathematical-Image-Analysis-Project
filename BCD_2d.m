...
function [Mu, alpha, B1, B2, history] = BI_BayCD_2d_iid( F_1d, Y, D, c, d )
    ...
    fun_FTAF = @(X,alpha) reshape((alpha*( F_1d'*( ( F_1d*X*(F_1d') ) )*F_1d )), [], 1); % function that gives the value of F^TAFx 
    ...
    fun_b = @(Y,alpha) reshape((alpha*( F_1d'*Y*F_1d )), [], 1); % function that gives the value of b = F^TAy 
    ...
    for counter = 1:MAX_ITER
        ...
        alpha = ( m^2 + 2*c )./( norm( reshape((( F_1d*Mu*(F_1d')- Y )), [], 1) )^2 + 2*d );   
