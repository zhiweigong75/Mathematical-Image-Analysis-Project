...
function [X, history] = ADMM_2d( F_1d, Y, D, lambda, rho, alpha, QUIET) 
    ...
    fun_FTF = @(X)  reshape((F_1d'*( ( F_1d*X*(F_1d') ) )*F_1d), [],1) ; % function that gives the value of F^TAFx 
    %fun_FTF = fun_FTF(:);
    fun_RTR = @(X,rho) reshape((rho*(  D'*( D*X )  +  ( X*(D') )*D )), [],1); % function that gives the value of R^TBRx
    %fun_RTR = fun_RTR(:);
    fun_G = @(X,rho) fun_FTF(X) + fun_RTR(X,rho); % function that gives value of Gx
    fun_b = @(Y,rho,V1,V2) reshape((( F_1d'*Y*F_1d ) + rho*( V1*D + D'*V2 )), [], 1); % function that gives the value of b = F^TAy 
    %fun_b = fun_b(:);
    ...
    for counter = 1:MAX_ITER
        ...
        aux1 = alpha*(X*(D')) + (1-alpha)*(Z1);
        aux1 = aux1(:);
        u1 = U1(:);
        z1 = shrinkage(aux1 + u1, lambda/rho);
        Z1 = reshape(z1,n,k); 
        aux2 = alpha*(D*X) + (1-alpha)*(Z2);
        aux2 = aux2(:);
        u2 = U2(:);
        z2 = shrinkage(aux2 + u2, lambda/rho);
        Z2 = reshape(z2,k,n); 
